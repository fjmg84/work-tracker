...
export function assertRepo(
  ...
  if (!/^[^/\s]+\/[^/\s]+$/.test(value)) {
    throw new Error(
      `Parámetro inválido: ${name} debe tener formato usuario/repo.`,
    );
  }
}
...
export function assertIdArray(
  ...
  if (
    !Array.isArray(value) ||
    value.some((v) => !Number.isInteger(v) || v <= 0)
  ) {
    throw new Error(
      `Parámetro inválido: ${name} debe ser una lista de enteros positivos.`,
    );
  }
}
